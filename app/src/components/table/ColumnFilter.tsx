...
import type { ColumnDef, ColumnFilterValue } from './types'
...
const empty = (col: ColumnDef): ColumnFilterValue => {
...
const isActive = (v: ColumnFilterValue | undefined) => {
  if (!v) return false
...
export function ColumnFilter({ col, options, value, onChange }: Props) {
      ...
      {open && (
        ...
        >
          <div className="flex items-center justify-between mb-2">
            <span className="text-xs font-medium text-white/80">Filtrar: {col.label}</span>
            {active && (
              <button
                onClick={clear}
          ...
          {v.kind === 'multi' && (
            <div>
              ...
              <div className="mt-2 max-h-56 overflow-y-auto">
                ...
                {options.length === 0 && (
                  <div className="p-2 text-xs text-[var(--color-muted)]">Sem opções.</div>
                )}
              </div>
            </div>
          )}
